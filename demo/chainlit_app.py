...
Uses ``data/catalog.jsonl`` and default live NLU. The Eval composer button
opens a local-evaluator dock over ``data/public_set.jsonl`` when the kit
``evaluator/`` package is present (demo-only; the agent package does not
read those labels). Chainlit APP_ROOT is ``demo/``.

Contest zip (this package as cwd):
...
Manual launch from ``demo/``:

    python -m chainlit run chainlit_app.py -w --port 8006

Imports ``src.Agent``. Each circuit stage reveal also prints turn_delta /
session / router / retrieve top-10 / decide+respond on this terminal.
"""
...
from src.paths import DATA_DIR, PROJECT_ROOT, SUBMISSION_ROOT

for _path in (str(PROJECT_ROOT), str(SUBMISSION_ROOT)):
...
from src.progress import progress_listener
...
from src.decide.clarification.utility import DEFAULT_SLIDER_POSITION

_FULL_CATALOG = DATA_DIR / "catalog.jsonl"
