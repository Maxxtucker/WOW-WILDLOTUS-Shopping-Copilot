...
_EVAL_MISSING = (
    "Eval dock needs the kit evaluator/ package and data/public_set.jsonl."
)
...
try:
    ...
    def catalog_index(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError(_EVAL_MISSING)

    def coarse_category(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError(_EVAL_MISSING)

    def customer_reply(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError(_EVAL_MISSING)

    def evaluate(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError(_EVAL_MISSING)

    def initial_message(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError(_EVAL_MISSING)

    def load_jsonl(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError(_EVAL_MISSING)

    def materialize_hidden_fields(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError(_EVAL_MISSING)

    def metric_summary(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError(_EVAL_MISSING)

    def normalize_recommendations(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError(_EVAL_MISSING)

    def parse_llm_mode(value: object = None) -> str:
        text = str(value or "remote").strip().lower()
        return text or "remote"

    def resolve_buyer_llm_backend(*_args: Any, **_kwargs: Any) -> Any:
        raise RuntimeError(_EVAL_MISSING)

    class OpenAICompatibleClient:  # type: ignore[no-redef]
        @classmethod
        def from_environment(cls) -> None:
            return None

    class ScenarioEvaluator:  # type: ignore[no-redef]
        pass
...
def kit_eval_available() -> bool:
    """True when the official evaluator and public_set are on disk."""

    return EVALUATOR_AVAILABLE and PUBLIC_SET_PATH.is_file()
