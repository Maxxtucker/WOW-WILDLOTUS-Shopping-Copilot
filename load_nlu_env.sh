#!/usr/bin/env bash
# Load src/assets/nlu.env into this shell. Does not write user/system env.
# Contest zip:  source load_nlu_env.sh
# Kit:          source scripts/load_nlu_env.sh
# Do not use `set -e` here: this file is meant to be sourced.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
ENV_FILE="$SCRIPT_DIR/src/assets/nlu.env"
if [[ ! -f "$ENV_FILE" ]]; then
