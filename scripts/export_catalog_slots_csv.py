...
from agent.retrieve.catalog.slots_sidecar import DEFAULT_SLOTS_RELATIVE
...
def main() -> None:
    ...
    parser.add_argument(
        "--sqlite",
        type=Path,
        default=ROOT / DEFAULT_SLOTS_RELATIVE,
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "data" / "product_slots.csv",
    )
    parser.add_argument(
        "--meta-output",
        type=Path,
        default=ROOT / "data" / "product_slots_meta.csv",
    )
    args = parser.parse_args()
    if not args.sqlite.is_file():
        raise SystemExit(
            f"Sidecar not found: {args.sqlite}. Run scripts/extract_catalog_slots.py first."
        )
