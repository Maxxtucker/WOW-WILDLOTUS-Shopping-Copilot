...
This chat runs `src.orchestrator.Agent` and the same production
`TurnPipeline` used by the headless API:

- full `data/catalog.jsonl`
- default live NLU from `src/assets/nlu.env` (first run: package-root `setup.ps1` /
  `setup.sh`, or kit `scripts/setup.ps1` / `python scripts/bootstrap.py --extras demo --run demo`);
