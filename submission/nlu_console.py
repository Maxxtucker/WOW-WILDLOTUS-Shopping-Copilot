...
From this directory (contest zip) or kit ``python submission/nlu_console.py``:
...
from src.pipeline import TurnPipeline
from src.retrieve.catalog import CatalogRetriever
from src.retrieve.catalog.index_path import resolve_index_path
from src.retrieve.from_slots import exact_pool_groups
from src.trace import TRACE_TOP, TurnTrace, build_router_trace, build_retrieve_trace
from src.understand.mode import MODE_NLU, MODE_REGEX, configure_understand
from src.understand.observation.hybrid import extract_from_regex, regex_is_high_confidence
from src.understand.observation.llm_nlu import (
    OllamaNluClient,
    load_nlu_env,
    set_nlu_client,
)
from src.understand.observation.runtime import ensure_llm_runtime
from src.understand.observation.schema import ObservationExtract
from src.understand.observation.slots import collect_failures
from src.understand.state import SessionState
...
def labeled_constraints(values: tuple[str, ...] | list[str]) -> list[dict[str, str]]:
    ...
    from src.domain import classify_constraint
...
def main() -> None:
    ...
    parser.add_argument(
        "--catalog",
        default=str(_PROJECT / "data" / "catalog.jsonl"),
        help="Catalog JSONL for the full pipeline (default: project data/catalog.jsonl).",
    )
    parser.add_argument(
        "--no-retrieve",
        action="store_true",
